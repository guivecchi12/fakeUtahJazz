  ...
  return(
    <div id="contact_us" className='flex flex-col px-5 py-5 text-white md:text-2xl md:py-20 md:px-40 bg-neutral-800 md:flex-row'>
        <div className='md:w-1/2'>
          ...
          {/* Email */}
          <Link  href="mailto:bizdev@sports.com">
          ...
          <p className='w-4/5 md:w-full'>
            With options like single-game tickets, suite rentals and special group rates, experiencing Utah Sports basketball has never been more convenient! Just let us know what you’re interested in and we can customize a package to fit your group and budget.
          </p>
        </div>
        <form onSubmit={formSubmit} className='flex flex-col w-1/2 mt-7 md:mt-0 md:w-2/5 md:pl-20'>
          ...
          <textarea
            ...
            required
          />
          <button className='flex items-center px-4 py-1 my-3 text-base text-black uppercase duration-300 ease-in bg-yellow-300 border border-black rounded Sports w-fit hover:text-black hover:bg-white'>Submit</button>
        </form>
      </div>
  )
}
